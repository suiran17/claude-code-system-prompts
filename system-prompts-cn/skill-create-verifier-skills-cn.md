...
description: 用于为 Verify 智能体创建验证器技能，以实现代码更改的自动验证
ccVersion: 2.1.51
-->
...
创建一个或多个验证器 (Verifier) 技能，供验证 (Verify) 智能体用于自动验证此项目或文件夹中的代码更改。如果项目有不同的验证需求（例如同时包含 Web UI 和 API 端点），您可以创建多个验证器。
...
1. **验证器名称** —— 根据检测结果建议一个名称，但由用户选择：
   ...
   允许自定义名称，但**必须**在名称中包含 "verifier" —— 验证智能体会通过查找文件夹名称中的 "verifier" 来发现技能。
...
2. 验证智能体如何发现它们 —— 文件夹名称必须包含 "verifier"（不区分大小写）以便自动发现
3. 以后可以编辑技能进行自定义
