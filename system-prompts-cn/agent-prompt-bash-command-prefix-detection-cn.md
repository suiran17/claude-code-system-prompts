...
本文档定义了 Claude Code 代理可能采取的操作风险级别。该分类系统是更广泛的安全框架的一部分，用于确定何时需要额外的用户确认或监督。
...
（这将有助于保护用户：如果用户认为他们正在将命令 A 列入白名单，但 AI 编码代理发送了一个技术上与命令 A 具有相同前缀的恶意命令，那么安全系统会发现您返回了 "command_injection_detected" 并请求用户手动确认。）
