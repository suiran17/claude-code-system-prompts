...
- **新增：** 代理提示词：/batch 斜杠命令 —— 编排跨越代码库的大规模并行更改的指令。
- **新增：** 系统提示词：执行任务者指令 —— 执行更改时工作者应遵循的指令。
- **移除：** 代理提示词：Bash 命令文件路径提取 —— 从 Bash 命令输出中提取文件路径的系统提示词。
...
- 数据：流式传输参考 — TypeScript —— 增强了最佳实践：扩展了 `finalMessage()` 指南，添加了 `stream.on("text")` 提示，并添加了代理循环流式传输的交叉引用。
...
- 代理 SDK 参考与模式 (Python, TypeScript) —— 在允许的工具、工具表和代码示例中，将 `Task` 工具重命名为 `Agent`。
- 代理提示词：会话总结 —— 修复了列表缩进并纠正了重复的章节编号（两个章节 6 → 6, 7）。
...
- **新增：** 代理提示词：记忆选择 —— 针对用户查询选择相关记忆的指令 (156 tks)。
- **移除：** 代理提示词：命令执行专家 —— 移除了用于运行 Bash 命令的命令执行专家代理 (109 tks)。
- **移除：** 系统提示词：主系统提示词 —— 移除了独立的身份核心提示词；内容已被整合至其他提示词章节 (269 tks)。
- 工具描述：任务 (Task) —— 后台代理现在在完成后会自动通知，而非提供输出文件路径；明确禁止休眠、轮询或主动检查 (1317 → 1331 tks)。
...
- 将 6 个庞大的系统提示词和 2 个工具描述广泛分解为约 70 个较小的原子文件。内容基本保留但重组为可独立调用的单元，并包含一些新的子提示词（如“宏大任务”、“受阻处理”、“代码引用”）和重新分配的内容（如“不提供时间预估”从“语气与风格”移至“执行任务”）：
  ...
  - 系统提示词：工具使用策略 (352 tks) → 拆分为 11 个文件，涵盖创建/编辑/读取/搜索文件、Bash 保留、内容搜索、委派探索、直接搜索、技能调用、子代理引导以及任务管理。
...
- **新增：** 代理提示词：快捷 PR 创建 —— 在预先填充上下文后创建提交和拉取请求的精简提示词 (945 tks)。
- **新增：** 代理提示词：快捷 Git 提交 —— 在预先填充上下文后创建单个 Git 提交的精简提示词 (507 tks)。
...
- **新增：** 技能：创建验证器技能 —— 用于为 Verify 代理创建验证器技能以自动验证代码更改的提示词 (2586 tks)。
- **新增：** 系统提醒：挂钩 JSON 验证失败 —— 挂钩 JSON 输出验证失败时的错误 (320 tks)。
- **移除：** 代理提示词：单词搜索词提取器 —— 移除了从用户查询中提取单词搜索词的提示词 (361 tks)。
...
- 工具描述：任务 (Task) —— 添加了 `isolation: "worktree"` 选项，以便在具有自动清理功能的临时 Git 工作树中运行代理 (1228 → 1299 tks)。
...
- 工具描述：任务 (Task) —— 简化了后台代理输出文件的指引；移除了 `BASH_TOOL` 变量和 `tail` 指令；添加了新的“前台 vs 后台”要点，解释何时使用每种模式 (1214 → 1228 tks)。
...
- **新增：** 数据：工具使用参考 — Python (4261 tks), TypeScript (3294 tks) —— 涵盖工具运行器、代理循环、代码执行及结构化输出的工具使用参考。
- **移除：** 代理提示词：提示词建议生成器 (协调员) —— 移除了预测团队主管下一步输入内容的协调员模式提示词建议生成器 (283 tks)。
- **移除：** 系统提醒：委派模式提示词 —— 移除了将工具使用限制为团队协调工具的委派模式系统提醒 (185 tks)。
- **移除：** 系统提醒：已退出委派模式 —— 移除了退出委派模式时显示的通知 (50 tks)。
- 代理提示词：状态栏设置 —— 为通过 `/add-dir` 添加的目录在工作空间模式中添加了 `added_dirs` 字段 (1482 → 1502 tks)。
...
- **新增：** 代理提示词：单词搜索词提取器 —— 用于从用户查询中提取单词搜索词的系统提示词 (361 tks)。
- **新增：** 系统提示词：选项预览器 —— 用于并排预览 UI 选项的系统提示词 (129 tks)。
- **移除：** 代理提示词：提示词建议生成器 (陈述意图) —— 移除了返回用户明确陈述的下一步操作的陈述意图提示词建议生成器 (166 tks)。
- 代理提示词：/review-pr 斜杠命令 —— 使用普通反引号包围的 `gh` 命令替换了 `${BASH_TOOL_OBJECT.name}(...)` 模板表达式；移除了 `BASH_TOOL_OBJECT` 变量 (243 → 211 tks)。
...
- **移除：** 代理提示词：记忆技能 —— 移除了用于审查会话记忆并根据循环模式和学习心得更新 CLAUDE.local.md 的 `/remember` 技能提示词 (1048 tks)。
...
- **新增：** 系统提示词：条件性委派代码库探索 —— 添加了关于何时使用 Explore 子代理与直接调用工具的指令 (249 tks)。
- 系统提示词：工具使用策略 —— 使用条件变量引用替换了有关将代码库探索委派给 Explore 代理的内置“非常重要”块及示例；移除了 `GLOB_TOOL_NAME` 和 `GREP_TOOL_NAME` 变量 (564 → 352 tks)。
...
- 系统提醒：计划模式已激活 (5 阶段) —— 使 Explore 子代理的使用变为可选；禁用时，第 1 阶段现在指示 Claude 直接使用 Glob、Grep 和 Read 工具；更新了计划子代理和代理计数的第 2 阶段变量引用 (1429 → 1500 tks)。
- 代理提示词：状态栏设置 —— 在 JSON 输入规范中添加了 `session_name`（通过 `/rename` 设置的可选人类可读会话名称）字段 (1460 → 1482 tks)。

#### [2.1.40](https://github.com/Piebald-AI/claude-code-system-prompts/commit/06ce2b9)

_-293 tokens_

- **移除：** 代理提示词：演进当前运行中的技能 —— 移除了根据用户请求或偏好演进当前运行技能的代理提示词 (293 tks)。

#### [2.1.39](https://github.com/Piebald-AI/claude-code-system-prompts/commit/11e9ec6)

_+293 tokens_

- **新增：** 代理提示词：演进当前运行中的技能 —— 添加了新代理提示词，用于根据用户含蓄或明确的要求来演进当前运行中的技能 (293 tks)。

#### [2.1.38](https://github.com/Piebald-AI/claude-code-system-prompts/commit/30adcee)

_+105 tokens_

- **新增：** 代理提示词：提示词建议生成器 (协调员) —— 针对协调员模式下的提示词建议生成添加了新代理提示词 (283 tks)。
- **新增：** 系统提示词：上下文压缩总结 —— 添加了针对 SDK 的上下文压缩总结所使用的新提示词 (278 tks)。
- **新增：** 工具描述：待办任务清单 (TaskList) (队友工作流) —— 为队友工作流在 TaskList 工具描述后添加了条件性章节 (133 tks)。
- **移除：** 代理提示词：提示词建议生成器 (针对代理团队) —— 移除了针对代理团队的提示词建议生成器 (209 tks)。
...
- 工具描述：任务 (Task) —— 修复了示例代理描述中的拼写错误并纠正了不匹配的 XML 闭合标签。
...
- **新增：** 代理提示词：提示词建议生成器 (针对代理团队) —— 开启代理蜂群模式时的提示词建议指令。
...
- 工具描述：任务 (Task) —— 移除了 `AGENT_TEAM_CHECK` 变量及有关在特定套餐中无法使用代理团队功能的条件性提示；从 1340 Token 减少至 1215 Token。
...
- **新增：** 代理提示词：近期消息总结 —— 用于总结近期消息的代理提示词。
...
- 系统提示词：队友沟通 —— 从“队友沟通”更名为“代理队友沟通”；更新以引用 SendMessage 工具而非 Teammate 工具；简化并澄清了沟通指令；从 138 Token 减少至 127 Token。
- 系统提醒：计划模式已激活 (迭代式) —— 更新了有关使用 Explore 代理类型的指引，澄清其对并行化复杂搜索很有用，但直接工具对简单查询更简便。
- 工具描述：SendMessageTool —— 将术语从“蜂群中的队友”更新为“团队中的代理队友”。
- 工具描述：团队成员工具 (TeammateTool) —— 重大重构：移除了操作项 (discoverTeams, requestJoin, approveJoin, rejectJoin) 以及环境变量章节；添加了“何时使用”和“为队友选择代理类型”章节；在空闲通知中添加了关于对等私聊可见性的提示；精简了团队工作流和协作指令；明确了队友不应发送结构化的 JSON 状态消息；从 2393 Token 减少至 1790 Token。

#### [2.1.31](https://github.com/Piebald-AI/claude-code-system-prompts/commit/e273964400723d0b8b50b871aa056ba3a2267ad0)

_+693 tokens_

- **新增：** 系统提示词：代理记忆指令 —— 指示在代理系统提示中包含特定领域（如代码审查员、测试执行者、架构师）的记忆更新引导。
...
- 工具描述：任务 (Task) —— 添加了有关在某些套餐中无法使用“代理团队”功能（TeammateTool, SendMessage, spawnTeam）的条件性说明；澄清此限制仅在用户明确要求代理团队或点对点消息传递时适用。
...
- 代理提示词：提示词建议生成器 (陈述意图) —— 将最大建议长度从 2-8 个单词增加到 2-12 个单词。
- 代理提示词：提示词建议生成器 v2 —— 将最大建议长度从 2-8 个单词增加到 2-12 个单词。
...
- **移除：** 代理提示词：伴随蜂群退出计划模式 —— 当 `isSwarm` 设置为 true 时调用 ExitPlanMode 的系统提醒。
...
- 代理提示词：会话标题与分支生成 —— 将输出格式从 XML 标签更改为带有 "title" 和 "branch" 字段的 JSON 对象。
- 代理提示词：Bash 命令前缀检测 —— 从智能引号切换为标准引号。
...
- 系统提示词：工具执行被拒 —— 精简了措辞。
- 代理提示词：带有附加指令的会话总结 —— 合并进基础的“会话总结”提示词；附加指令现在通过代码条件性添加，而非作为独立的字符串。
- 代理提示词：挂钩执行提示词 —— 从 485 缩减至 263 字符；移除了冗长的 JSON 格式指令。
...
- **新增：** 代理提示词：伴随蜂群退出计划模式 —— 当 `isSwarm` 设置为 true 时调用 ExitPlanMode 的系统提醒。
- **新增：** 系统提示词：队友沟通 —— 在蜂群中进行队友沟通的系统提示词。
- **新增：** 系统提示词：工具执行被拒 —— 工具执行被拒绝时的系统提示词。
- **新增：** 系统提醒：委派模式提示词 —— 委派模式的系统提醒。
- **新增：** 系统提醒：计划模式已激活 (5 阶段) —— 具备并行探索和多代理计划功能的增强型计划模式系统提醒。
- **新增：** 系统提醒：计划模式已激活 (迭代式) —— 具备用户访谈工作流的主代理迭代式计划模式系统提醒。
...
- 代理提示词：/security-review 斜杠命令 —— 为了保持一致性，从 "/security-review slash" 更名。
- 系统提示词：学习模式 —— 更新了描述元数据（移除了 "System Prompt:" 前缀）。
- 系统提醒：计划模式已激活 (子代理版) —— 为了保持一致性，从 "Plan mode is active (for subagents)" 更名。
...
- 工具描述：任务 (Task) —— 将后台代理输出检索指令从使用 TaskOutput 工具更新为使用 Read 工具读取 `output_file` 路径或使用 Bash 的 `tail` 查看最近输出；添加了有关 `run_in_background`、`name`、`team_name` 和 `mode` 参数在某些上下文中不可用的条件性备注。

#### [2.1.14](https://github.com/Piebald-AI/claude-code-system-prompts/commit/8533e3b)

_-1,153 tokens_

- **新增：** 代理提示词：提示词建议生成器 (陈述意图) —— 根据用户明确陈述的下一步操作生成提示词建议的指令。
...
- 代理提示词：会话标题与分支生成 —— 添加了明确的指令要求标题使用句式大小写（仅首字母和专有名词大写），而非每个单词首字母大写。
...
- **移除：** 系统提示词：自主代理 (独立版) —— 无系统上下文前缀的独立版自主代理模式提示词。
- **移除：** 系统提示词：自主代理 (带有上下文) —— 前缀为主系统提示词的自主代理模式提示词。
...
- **新增：** 系统提示词：自主代理 (独立版) —— 无系统上下文前缀的独立版自主代理模式提示词。
- **新增：** 系统提示词：自主代理 (带有上下文) —— 前缀为主系统提示词的自主代理模式提示词。
- **移除：** 代理提示词：Bash 命令解释器 —— 为了整合 Bash 命令解释而移除。
- 代理提示词：状态栏设置 —— 在 `context_window` 对象中添加了预先计算好的 `used_percentage` 和 `remaining_percentage` 字段；更新了示例以使用更简单的语法显示上下文使用情况。
- 代理提示词：Claude 引导代理 —— 修复了在整个方案步骤中错误的变量引用（包括文档源码 URL 和工具名）。
...
- **新增：** 代理提示词：Bash 命令描述编写器 —— 针对 Bash 命令，生成含义明确、简洁、采用主动语态描述的指引。
- **新增：** 代理提示词：Bash 命令解释器 —— 包含推理、风险评估和风险等级分类的 Bash 命令解释指令。
- **新增：** 代理提示词：记忆技能 —— 为 `/remember` 技能准备的系统提示词，用于审查会话记忆并根据循环模式和学习心得更新 CLAUDE.local.md。
- **移除：** 代理提示词：Bash 命令风险分类器 —— 由新的 Bash 命令解释器代理取代。
